...
def check_the_possible_mapping(df, work_time, day, df_element):
	...
	col_name_list = ['left_time','work_time','real_left_time']
	for j in range(1,result_df.shape[1]-2):
		...
		for w_idx, work in enumerate(T_df.columns[::-1]):
			...
			if visited[l_idx] ==False and visited[w_idx] ==False:
				if left_t > work_t and left != work:		
					...
					matched_list.append([work_time - (left_t+work_t), left, work])
	

	for i, v  in enumerate(T_df.columns):
		if visited[i]==False:
			matched_list.append([left_time_dict[v+"-left"] , v])	
