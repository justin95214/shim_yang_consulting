	...
	def route(self, temp_df, df_element,day, current_point, df_info):
		...
		while (len(temp_df.loc[temp_df['stopby']==0]) >0):
			...
			if route_check == False:
				...
				#temp_df['deg'] = [ x - 180 if x > 180 and x< 360 else x for x in temp_df['deg']]
				...
				Max_deg = max(temp_df['deg'].values.tolist())
				#temp_df['deg'] = temp_df['deg']/Max_deg
				

				Max_dist = max(temp_df[current_point+'_운행거리'].values.tolist())

				temp_df.loc[temp_df[current_point+'_운행거리']<=0.5,'deg']=0

				#temp_df[current_point+'_운행거리'] = temp_df[current_point+'_운행거리']/(Max_dist)
				

				temp_df['new_std'] = (temp_df['deg']/Max_deg) + (temp_df[current_point+'_운행거리']/Max_dist)
