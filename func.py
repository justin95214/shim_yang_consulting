...
def simulation(DF_client, DP, Df_info):
    client = DF_client
    dp = DP
    df_info = Df_info
    ...
    for day in week_num:
        for DF_element in df_DP['DP'].values.tolist():
            ...
            temp_df[day] = temp_df[day].replace(np.NaN, 0)

            temp_df = temp_df[temp_df[day] > 0].copy()
            ...
            # result_df = pd.read_csv("./test2/" + day + "_" + DF_element + "_result.csv", encoding='euc-kr')
            work_time = df_info[df_info['DP'] == DF_element]['대당 운행시간(분)'].values.tolist()[0]

            result_df_T, im_result_df_T, result_df, im_result_df = mapping.check_the_possible_mapping(result_df, work_time,
                                                                                                      day, DF_element)

            map1_df = mapping.mapping1(result_df_T, im_result_df_T, day, DF_element, result_df, im_result_df)
